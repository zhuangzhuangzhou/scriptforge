...
### 8.13 子字符串匹配陷阱（LLM 响应解析）

**问题**: 使用 `in` 操作符检查关键词时，会匹配到包含该关键词的任何文本，导致误判

**场景**: 解析 LLM 响应时，检测空响应关键词如"没有"、"无"、"空"等
...
| 响应内容 | 预期结果 | 实际结果 |
|---------|---------|---------|
| `没有更多冲突` | 解析为剧情点 | ❌ 返回空数组（匹配"没有"）|
| `场景无明确标识` | 解析为剧情点 | ❌ 返回空数组（匹配"无"）|
| `[]` | 返回空数组 | ✅ 正确 |
...
**预防措施**:
1. **精确匹配**: 检查响应是否完全等于特定值，而不是包含特定值
2. **白名单思路**: 明确知道什么是"空"，而不是猜测什么可能是"空"
3. **测试用例**: 添加包含关键词的正常响应作为测试用例
4. **日志记录**: 在解析失败时记录响应内容，便于排查
...
**实际案例**:
- 2026-02-26: 修复 `parse_llm_response` 函数，剧情拆解成功但返回 EMPTY_RESULT 错误

...
## [20260226-232740] 修复批量拆解时批次不自动切换的问题

**时间**: 2026-02-26 23:27:40
...
**摘要**: 修复批量拆解时第一批次完成后不会自动切换到第二批次的问题。借鉴 Script Tab 的成功实践，采用精准刷新机制替代轮询。

---


## [20260227-222302] 修复登录和模型配置问题

**时间**: 2026-02-27 22:23:02

**提交**:
- `ae2972f` - fix: 邮箱登录支持和模型配置自动选择默认值
...
- 使用 `or_` 条件同时查询 `username` 和 `email` 字段
- 用户现在可以使用用户名或邮箱登录

### 2. 项目配置模型自动选择
- 修改文件: `frontend/src/pages/user/Workspace/ConfigTab/index.tsx`
- 模型列表加载完成后，自动选择第一个可用模型作为默认值
- 解决旧项目模型配置为空时保存不生效的问题

## 技术要点

| 问题 | 解决方案 |
|------|----------|
| 邮箱登录无效 | SQLAlchemy `or_` 多字段查询 |
| 模型配置保存失败 | 自动选择默认值，避免空值提交 |
